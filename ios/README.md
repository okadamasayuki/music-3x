...
- **音程の維持**: 4x でも声が高くなりません。オフにすると `.varispeed`
  (テープ早回し相当)に切り替わります。
- **音の伸ばし方**: 音程を保つ方式は聞こえ方がはっきり変わるため、画面から選べます。
  「話し声」は `.timeDomain` で子音の輪郭が残り、YouTube 等の倍速に近い聞こえ方。
  「音楽」は `.spectral` で音の質感を保ちますが、話し声では輪郭がぼやけがちです。
  方式を変えたときは同じ位置へシークし直します。既に読み込まれた音には
  設定変更が反映されないためです。
